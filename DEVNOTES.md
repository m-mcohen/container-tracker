...
| App version (shown in title bar + update check) | `core/constants.py` | `__version__` (single source of truth — installer/pyproject/About all derive from it; see Build and release) |
...
- `__version__` — shown in the window title and About panel. The installer (`version.iss`, generated by `build.bat`) and `pyproject.toml` (patched by `tools/release.ps1`) derive from it — never hand-edit those.
...
`container_tracker/core/constants.py`. `build.bat` generates `version.iss`
from it (which `installer.iss` `#include`s), `tools/release.ps1` patches
`pyproject.toml` in lockstep, and the About panel reads it at runtime via
`get_settings()["app_version"]`.

```powershell
.\tools\release.ps1 -Version X.Y.Z   # bump -> pytest -> exe -> installer -> tag -> GitHub Release
.\tools\release.ps1 -Version X.Y.Z -BuildOnly   # local build only, no git/GitHub
```

Manual fallback:

```bat
build.bat                  :: builds dist\ContainerTracker.exe + version.iss
iscc installer.iss         :: builds dist\installer\ContainerTracker_Setup_v<version>.exe
```

Build deps install from `requirements-build.txt` into the Python 3.12 the
`py` launcher resolves. The update banner compares the `releases/latest`
tag against `__version__` — the repo/releases must be publicly reachable
(the check is unauthenticated).
